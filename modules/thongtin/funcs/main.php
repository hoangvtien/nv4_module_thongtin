...
if($madvi == 0)
{
    $sql2 = "SELECT tv.id, tv.hoten, tv.ngsinh, tv.gtinh, tv.avt, tv.diachi, tv.email, dv.tendonvi, cv.tenchucvu FROM " 
       . NV_PREFIXLANG . "_" . $module_data .
       " tv INNER JOIN " . NV_PREFIXLANG . "_" . $module_data . "_donvi dv
       INNER JOIN " . NV_PREFIXLANG . "_" . $module_data . "_chucvu cv                  
       ON tv.madvi = dv.madvi AND tv.macvu1 = cv.macvu WHERE 1 = 1 AND tv.hoten like '%" . $key . "%' LIMIT " . $page . ", " . $module_config['per_page'] . "";
}
else
{
    $sql2 = "SELECT tv.id, tv.hoten, tv.ngsinh, tv.gtinh, tv.avt, tv.diachi, tv.email, dv.tendonvi, cv.tenchucvu FROM " 
       ...
       " tv INNER JOIN " . NV_PREFIXLANG . "_" . $module_data . "_donvi dv
       INNER JOIN " . NV_PREFIXLANG . "_" . $module_data . "_chucvu cv                  
       ON tv.madvi = dv.madvi AND tv.macvu1 = cv.macvu WHERE tv.madvi = " . $madvi . " LIMIT " . $page . ", " . $module_config['per_page'];
}
...
{
	if( empty( $thongtin['avt'] ) )
	{
		$thongtin['avt'] = NV_BASE_SITEURL . "themes/" . $module_info['template'] . "/images/thongtin/no-image.jpg";
	}
    $xtpl->assign('THONGTIN', $thongtin);
    

	$xtpl->assign( 'DETAIL', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name  . "&amp;" . NV_OP_VARIABLE . "=detail/" . $thongtin['id']."-".change_alias($thongtin['hoten']));
	
            
    //Bat tat dia chi tai toplip
    if(!empty($thongtin['diachi']))
    {
